...
public class VATSpawner : MonoBehaviour
{
    ...
    [Range(0.5f, 2f)]
    public float MinPlaybackSpeed = 0.8f;
    [Range(0.5f, 2f)]
    public float MaxPlaybackSpeed = 1.2f;
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < SpawnCount; i++)
        {
            ...
            {
                CurrentFrame = random.NextFloat(0f, FrameCount),                        // 랜덤 시작 프레임
                FrameCount = FrameCount,
                Fps = Fps,
                PlaybackSpeed = random.NextFloat(MinPlaybackSpeed, MaxPlaybackSpeed),     // 랜덤 재생 속도
                IsLooping = IsLooping
            });
